...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  let token;
  // Incoming token will be in a header called Authorization which starts with the word "Bearer"
  if (
  ...
  // Storing current user into req.user so we have the needed data for the next middleware
  req.user = currentUser;
  next();
});

// Passing arguments into middleware function by wrapping it and creating a closure
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  // 1. Get user based on received token
